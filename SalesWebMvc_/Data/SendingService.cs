...
using SalesWebMvc_.Models.Enums;

namespace SalesWebMvc_.Data
{
    public class SendingService
    {
        private SalesWebMvc_Context _context;
        
        public SendingService(SalesWebMvc_Context context)
        {
            _context = context;
        }
        ...
        public void Seed()
        {
            //verifico se há algum registro nas tabelas
            if (_context.Departament.Any() ||
                _context.Seller.Any() ||
                _context.SalesRecord.Any())
            {
                return; //se houver registros na tabela, encerra a execução do método
            }
            else
            {
